...
```sh
curl -fsSL https://raw.githubusercontent.com/steekell/dotfiles/v0.1.17/install.sh | sh
```
...
```powershell
irm https://raw.githubusercontent.com/steekell/dotfiles/v0.1.17/install.ps1 | iex
```
...
| `DOTFILES_REF` | `v0.1.17` | Branche ou tag git (override pour `main` en dev) |
...
- **macOS** dépendances hors brew (manuel, une fois) :
  1. Driver **Karabiner-DriverKit-VirtualHIDDevice v0.1.17** (pkg pqrs ; pas un brew package kanata)
