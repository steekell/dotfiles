...
```sh
curl -fsSL https://raw.githubusercontent.com/steekell/dotfiles/v0.2.2/install.sh | sh
```
...
```powershell
irm https://raw.githubusercontent.com/steekell/dotfiles/v0.2.2/install.ps1 | iex
```
...
| `DOTFILES_REF` | `v0.2.2` | Branche ou tag git (override pour `main` en dev) |
...
- **macOS** dépendances hors brew (manuel, une fois) :
  1. Driver **Karabiner-DriverKit-VirtualHIDDevice v0.2.2** (pkg pqrs ; pas un brew package kanata)
