...
-------------------------------------------------------------------------------
-- Sauvegarde / Quitter
map("n", "<leader>1",           "<cmd>qa<cr>",        { desc = "Quit All" })
map("n", "<leader>2",           "<cmd>w<cr><esc>",    { desc = "Save File" })
...
-- c : never goes to clipboard
map({ "n", "v" }, "c",       [["_c]],          { desc = "Change" })
map("n", "C",                [["_C]],          { desc = "Change to end of line" })
map("n", "cc",               [["_cc]],         { desc = "Change entire line" })
...
local function open_yazi()
  if vim.fn.executable("yazi") == 0 then
    vim.notify("yazi introuvable dans PATH — installer yazi ou utiliser oil (<leader>o)", vim.log.levels.WARN)
    return
  end
...
map("n",          "<leader>tt", function() term_bottom(vim.fn.getcwd()) end, { desc = "Terminal Bottom (cwd)" })
map("n",          "<leader>tr", function() term_bottom(git_root()) end,      { desc = "Terminal Bottom (git root)" })
map({ "n", "t" }, "<c-/>",      function() term_bottom(git_root()) end,      { desc = "Terminal Bottom (git root)" })
map({ "n", "t" }, "<c-_>",      function() term_bottom(git_root()) end,      { desc = "which_key_ignore" })
map("n",          "<leader>th", function() term_left(vim.fn.getcwd()) end,   { desc = "Terminal Left 33% (cwd)" })
map("n",          "<leader>tl", function() term_right(vim.fn.getcwd()) end,  { desc = "Terminal Right 33% (cwd)" })
...
map("n", "<leader>gh", fzf.git_bcommits, { desc = "Current File History (fzf)" })

map("n", "<leader>gb", fzf.git_branches, { desc = "Git Branches (fzf)" })
map("n", "<leader>gs", fzf.git_status,   { desc = "Git Modified Files (fzf)" })
...
local function sys_open(url)
  ...
  elseif vim.fn.executable("xdg-open") == 1 then
    ...
    vim.notify("Aucun ouvreur d'URL (open/xdg-open/start)", vim.log.levels.WARN)
  end
end
...
map("n", "<leader>ff", fzf.files,      { desc = "Find File (fzf)" })
map("n", "<leader>fg", fzf.live_grep,  { desc = "Grep in Project (fzf)" })
map("n", "<leader>fb", fzf.buffers,    { desc = "List Buffers (fzf)" })
map("n", "<leader>fh", fzf.help_tags,  { desc = "Neovim Help (fzf)" })
map("n", "<leader>fo", fzf.oldfiles,   { desc = "Recent Files (fzf)" })
map("n", "<leader>fr", fzf.resume,     { desc = "Resume Last Search (fzf)" })

-- Word under cursor
map("n", "<leader>fw", fzf.grep_cword,  { desc = "Search Word Under Cursor (fzf)" })
map("n", "<leader>fW", fzf.grep_cWORD, { desc = "Search WORD Under Cursor (fzf)" })

-- Lines
map("n", "<leader>fl", fzf.lines,       { desc = "Search in Open Buffers (fzf)" })
map("n", "<leader>fL", fzf.blines,      { desc = "Search in Current Buffer (fzf)" })

-- Neovim
map("n", "<leader>fc", fzf.commands,    { desc = "Neovim Commands (fzf)" })
map("n", "<leader>fk", fzf.keymaps,     { desc = "Keymaps (fzf)" })
map("n", "<leader>fm", fzf.marks,       { desc = "Vim Marks (fzf)" })
map("n", "<leader>fj", fzf.jumps,       { desc = "Jump List (fzf)" })
map("n", "<leader>fq", fzf.quickfix,    { desc = "Quickfix List (fzf)" })
map("n", "<leader>fd", fzf.diagnostics_document,  { desc = "Document Diagnostics (fzf)" })
map("n", "<leader>fD", fzf.diagnostics_workspace, { desc = "Workspace Diagnostics (fzf)" })

-- LSP (global)
map("n", "<leader>lS", fzf.lsp_workspace_symbols, { desc = "Workspace Symbols (fzf)" })
...
map("n", "<leader>o", oil_toggle_right,                           { desc = "Toggle Oil (right 25%)" })
map("n", "-",         "<cmd>Oil<cr>",                             { desc = "Oil (current directory)" })
map("n", "<leader>O", function() require("oil").open_float() end, { desc = "Oil (floating)" })
